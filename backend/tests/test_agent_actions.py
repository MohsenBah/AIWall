...
def test_extract_tool_calls_from_assistant_message() -> None:
    body = json.dumps(
        {
            "model": "gpt-4o-mini",
            "messages": [
    ...
    assert actions[0].action_type == ACTION_TOOL_CALL
    assert actions[0].action_target == "run_terminal"
...
def test_extract_legacy_function_call() -> None:
    body = json.dumps(
        {
            "model": "gpt-4o-mini",
            "messages": [
    ...
    actions = extract_agent_actions_from_body(body)
    assert len(actions) == 1
    assert actions[0].action_target == "get_weather"
...
def test_write_agent_action_persists_type_and_target(tmp_path: Path) -> None:
    ...
    assert len(rows) == 1
    assert rows[0].action_type == ACTION_TOOL_CALL
    assert rows[0].action_target == "read_file"

    listed = writer.list_agent_actions(request_id="req-1")
    assert len(listed) == 1
    assert listed[0].action_type == ACTION_TOOL_CALL
    assert listed[0].action_target == "read_file"
...
@pytest.mark.asyncio
async def test_proxy_stores_tool_call_action_type_and_target(
    tmp_path: Path,
    ...
    assert match.action_type == ACTION_TOOL_CALL
    assert match.action_target == "bash"
